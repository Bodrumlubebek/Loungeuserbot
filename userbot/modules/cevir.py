...
@register(pattern="^.convt ?(gif|audio|photo|mp3|voice)? ?(.*)", replyneeded=True)
async def cevir(event):
    ...
    if islem == "foto" or islem == "photo":
        ...
        await event.client.send_file(event.chat_id, "sticker.png", reply_to=rep_msg, caption="**@loungesupport ile Fotoğrafa çevirildi**")

        await event.delete()
        os.remove("sticker.png")
    elif islem == "ses" or islem == "audio":
        ...
        if efekt in EFEKTLER:
            ...
            await event.client.send_file(event.chat_id, "output.mp3", reply_to=rep_msg, caption="**@loungesupport ile efekt uygulandı.**")
        ...
        try:
            await event.client.send_file(event.chat_id, "out.mp3",reply_to=rep_msg, caption='**@loungesupport ile sese çevrildi.**')
        except:
            os.remove(video)
...
CmdHelp('cevir').add_command(
    'çevir photo', '<yanıt>', 'Stickeri fotoğrafa çevirir.'
).add_command(
    'çevir gif', '<yanıt>', 'Videoyu gife çevirir.'
).add_command(
    'çevir audio', '<çocuk/robot/earrape/hızlı/parazit/yankı>', 'Sese efekt uygular.'
).add_command(
